...
window.addEventListener('touchmove', updateRipple, { passive: true });
...
const timer = setInterval(() => {
    ...
    if (diff <= 0) {
        ...
        setTimeout(() => {
            ...
            window.removeEventListener('mousemove', updateRipple);
        }, 1500); 
        return;
    }
...
window.addEventListener("mousedown", (e) => {
  ...
  if (document.getElementById("message-section").style.display === "flex" && e.target.id !== "replayBtn") {
    ...
    if (currentStep === 11) { // Blow candle page
